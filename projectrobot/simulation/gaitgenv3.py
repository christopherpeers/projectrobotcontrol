...
horiz = 40.0 #From motor1 rotation point to that horizontal link 
...
T = 3.0 #Time period for gait, ie 4 leg movements or one step

stepsize = 30 #mm x distance of step
yratio = 1.0 #Semi-circle for simplicities sake. Height of step ratio to radius of step.
	...
	while not rospy.is_shutdown():
		...
		if (n == 1):
			...
			while(j < stepsize):
				x = traj[j][0]
				y = traj[j][1]
				anglearrayback[j] = inversekinematics(x,y,"rear")
				j = j + 1
			...
			while(j < stepsize):
				x = shift[j][0]
				y = shift[j][1]
				shiftanglesfront[j] = inversekinematics(x,y,"front")
				j = j + 1
			...
			while(j < stepsize):
				x = shift[j][0]
				y = shift[j][1]
				shiftanglesback[j] = inversekinematics(x,y,"rear")	
				j = j + 1
			...
			while(j < stepsize):
				x = traj[j][0]
				y = traj[j][1]
				anglearrayfront[j] = inversekinematics(x,y,"front")
				j = j + 1
			#resetf()
			#print xpos,ypos,zpos
			n = 2
...
def inversekinematics(x,y,leg): #Need to know where the robot thinks its feet are.
	global fxpos
	global fypos
	global rxpos
	global rypos
	...
	if leg == "rear":
		...
		inputx = rxpos
		inputy = rypos
		print inputx,inputy,fxpos,fypos,rxpos,rypos
